...
const getStatusColor = (status: Project['status']) => {
  switch (status) {
    case ProjectStatus.OPEN_APPLYING:
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        <span className="flex items-center">
                          ...
                          Đăng: {project.createdDate ? formatDate(project.createdDate) : 'Chưa xác định'}
                        </span>
                        <span className="flex items-center">
                          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          Deadline: {project.endDate ? formatDate(project.endDate) : 'Chưa xác định'}
                        </span>
                        <span className="flex items-center">
                          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex flex-col items-end space-y-2">
                      ...
                      </span>                    <div className="text-2xl font-bold text-blue-600">
                        {project.budget ? formatCurrency(project.budget) : 'Thỏa thuận'}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Skills */}
                  <div className="mb-4">
                    <h3 className="text-sm font-medium text-gray-700 mb-2">Kỹ năng yêu cầu:</h3>                  <div className="flex flex-wrap gap-2">
                  ...
                  <div className="p-6">
                    {activeTab === 'description' && (
                      <div className="prose max-w-none">
                        ...
                        <div className="mt-8 space-y-4">                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <h4 className="font-medium text-gray-900 mb-2">Thông tin dự án</h4>
                            <ul className="space-y-1 text-sm text-gray-600">
                              <li>• Loại dự án: {project.projectType?.name || 'Không xác định'}</li>
                              <li>• Thời gian dự kiến: {project.estimateDuration ? `${project.estimateDuration} ngày` : 'Linh hoạt'}</li>
                              <li>• Địa điểm: {project.location || 'Remote'}</li>
                              <li>• Danh mục: {project.category || 'Khác'}</li>
                            </ul>
                          </div>
                          <div>
                            <h4 className="font-medium text-gray-900 mb-2">Chi tiết ngân sách</h4>
                            <ul className="space-y-1 text-sm text-gray-600">
                              <li>• Ngân sách: {project.budget ? formatCurrency(project.budget) : 'Thỏa thuận'}</li>
                              <li>• Tối thiểu: {project.minBudget ? formatCurrency(project.minBudget) : 'Không giới hạn'}</li>
                              <li>• Tối đa: {project.maxBudget ? formatCurrency(project.maxBudget) : 'Không giới hạn'}</li>
                              <li>• Làm việc: {project.isRemote ? 'Remote' : 'Tại văn phòng'}</li>
                            </ul>
                          </div>
                        </div>
                        </div>
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                  ...
                  {project.status === ProjectStatus.OPEN_APPLYING   && !hasApplied && !isProjectOwner && (
                    <button
                  ...
                  {hasApplied && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                      ...
                      <button
                        onClick={() => router.push(`/chatbox?projectId=${id}&customerId=${project.customerId}&freelancerId=${user?.id}`)}
                      ...
                      <button
                        onClick={() => router.push(`/nda-contracts?projectId=${id}&type=project&partnerId=${project.customerId}`)}
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      {/* <span className="font-medium">{formatDate(project.)}</span> */}
                    </div>
                  </div>
                </div>
                    ...
                    {mockProjects.slice(0, 3).map(similarProject => (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-600 mb-2">
                          {formatCurrency(similarProject.budget || 0)}
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {similarProject.requiredSkills?.slice(0, 2).map(skill => (
